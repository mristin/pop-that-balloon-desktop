...
"""Check that the distribution and popthatballoon/__init__.py are in sync."""
...
def main() -> int:
    ...
    if setup_py_map["version"] != popthatballoon.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in popthatballoon/__init__.py is: "
            f"{popthatballoon.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != popthatballoon.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in popthatballoon/__init__.py is: "
            f"{popthatballoon.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != popthatballoon.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in popthatballoon/__init__.py is: "
            f"{popthatballoon.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != popthatballoon.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in popthatballoon/__init__.py is: "
            f"{popthatballoon.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != popthatballoon.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in popthatballoon/__init__.py, "
                f"but found: {popthatballoon.__status__}"
            )
            success = False
